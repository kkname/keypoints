...
from pcdet.ops.sptr.sptr.utils import to_3d_numpy, get_indices_params
from pcdet.ops.sptr.sptr.modules import sparse_self_attention, SparseTrTensor
import numpy as np
...
class SparseAttentionWrapper(spconv.SparseModule):
    ...
    def __init__(self, channels, num_heads, indice_key: str, window_size: int = 6, dropout=0., shift_win=False, attn_type="box"):
        ...
        if attn_type == "box":
            self.attn = modules.VarLengthMultiheadSA(
                embed_dim=channels,
                num_heads=num_heads,
    ...
    def forward(self, sptr_tensor: sptr.SparseTrTensor):
        ...
        if index_params is None:
            ...
            (index_0, index_0_offsets, n_max, index_1, index_1_offsets, sort_idx, window_size, shift_win, \
                index_0_sphere, index_0_offsets_sphere, n_max_sphere, index_1_sphere, index_1_offsets_sphere, sort_idx_sphere, window_size_sphere) = index_params
